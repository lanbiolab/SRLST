...
class train():
    def __init__(self,
        ...
        ############################
        self.adj_histology = histology_dict['adj_norm'].to(self.device)
        self.adj_label_histology = histology_dict['adj_label'].to(self.device)
        self.norm_histology = histology_dict['norm_value']
    ...
    def pretrain(
        ...
        with tqdm(total=int(self.pre_epochs),
                  desc="SRLST trains an initial model",
            ...
            for epoch in range(self.pre_epochs):
                ...
                z, mu, logvar, mu2, logvar2, de_feat, _, feat_x = self.model(Variable(inputs_coor), self.adj,
                                                                             self.adj_histology)
                preds1 = self.model.dc(z)
                preds2 = self.model.dc_histology(z)
                loss1 = self.model.deepst_loss(
                    preds=preds1,
                ...
                loss2 = self.model.deepst_loss(
                    preds=preds2,
                    labels=self.adj_label_histology,
                    mu=mu2,
                    logvar=logvar2,
                    n_nodes=self.num_spots,
                    norm=self.norm_histology,
                    mask=self.adj_label_histology,
                    bce_kld_weight=self.bce_kld_weight,
                )
                loss3 = self.model.decoded_loss(
                    decoded=de_feat,
                    x=self.data,
    ...
    def process(
        ...
        if self.domains is None:
            z, _, _, _, _, _, q, _, = self.model(self.data, self.adj, self.adj_histology)
        else:
            z, _, _, _, _, _, q, _, = self.model(self.data, self.adj, self.adj_histology)
    ...
    def fit(self,
        ...
        with tqdm(total=int(self.pre_epochs),
                  desc="SRLST trains a final model",
                  bar_format="{l_bar}{bar} [ time left: {remaining} ]") as pbar:
            for epoch in range(self.epochs):
                ...
                z, mu, logvar, mu2, logvar2, de_feat, out_q, feat_x = self.model(Variable(inputs_coor), self.adj, self.adj_histology)
                preds1 = self.model.dc(z)
                preds2 = self.model.dc_histology(z)
                ...
                loss = self.kl_weight * loss_kl + loss1 + loss2 + loss3
                print(loss)
                loss.backward()
