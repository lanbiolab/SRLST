...
from torch_geometric.nn import Sequential, BatchNorm, InstanceNorm, SGConv
from typing import Callable, Iterable, Union, Tuple, Optional
import logging


class SRLST_model(nn.Module):
    def __init__(self,
                 input_dim,
                 Conv_type='GCNConv',
        ...
        super(SRLST_model, self).__init__()
        ...
        current_decoder_dim = linear_encoder_hidden[-1] + 2 * conv_hidden[-1]
        ...
        if self.Conv_type == "GCNConv":
            ...
            self.conv1_mean = Sequential('x, edge_index', [
                (GCNConv(conv_hidden[0] * 2, conv_hidden[-1]), 'x, edge_index -> x1'),
            ])
            self.conv1_logvar = Sequential('x, edge_index', [
                (GCNConv(conv_hidden[0] * 2, conv_hidden[-1]), 'x, edge_index -> x1'),
            ])
            ###################################
            ...
            self.conv2_mean = Sequential('x, edge_index', [
                (GCNConv(conv_hidden[0] * 2, conv_hidden[-1]), 'x, edge_index -> x1'),
            ])
            self.conv2_logvar = Sequential('x, edge_index', [
                (GCNConv(conv_hidden[0] * 2, conv_hidden[-1]), 'x, edge_index -> x1'),
            ])
        ...
        elif self.Conv_type == "FeaStConv":  ## 平滑
        ...
        self.dc_histology = InnerProductDecoder(p_drop)
        # DEC cluster layer
        self.cluster_layer = Parameter(
            torch.Tensor(self.dec_cluster_n, self.linear_encoder_hidden[-1] + 2 * self.conv_hidden[-1]))
    ...
    def encode(
        ...
        conv1_x = self.conv1(feat_x, adj)
        conv2_x = self.conv2(feat_x, adj_histology)
        return self.conv1_mean(conv1_x, adj), self.conv1_logvar(conv1_x, adj), self.conv2_mean(conv2_x, adj_histology), self.conv2_logvar(conv2_x, adj_histology), feat_x
    ...
    def deepst_loss(
        ...
        return bce_kld_weight * (bce_logits_loss + KLD)
    ...
    def forward(
        ...
        mu, logvar, mu2, logvar2, feat_x = self.encode(x, adj, adj_histology)
        gnn1_z = self.reparameterize(mu, logvar)
        gnn2_z = self.reparameterize(mu2, logvar2)
        z = torch.cat((feat_x, gnn1_z, gnn2_z), 1)
        ...
        return z, mu, logvar, mu2, logvar2, de_feat, q, feat_x
