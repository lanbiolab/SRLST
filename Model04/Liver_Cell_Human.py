...
# sample name
data_path = "data/Liver_Cell_Human"  #### to your path
data_name = 'JBO18'  #### project name
save_path = "Results/liver"  #### save path
seed = 1024
n_domains = 4
fix_seed(seed)
train = True
if train:
    deepen = run(save_path=save_path,
                 ...
                 pre_epochs=500,  ####  choose the number of training
    ...
    df_meta = pd.read_csv(os.path.join(data_path, data_name, 'annot_humanVisium.csv'))
    position = pd.read_csv(os.path.join(data_path, data_name, 'spatial/tissue_positions_list.csv'), header=None,
                           index_col=0)
    df_meta = df_meta.loc[df_meta["sample"] == data_name]
    filtered = list(df_meta["spot"])
    ...
    adata = adata[adata.obs_names.isin(filtered), :].copy()
    adata.obs['layer_guess'] = list(df_meta['zonationGroup'])
    a = adata.obs['layer_guess']
    ...
    deepen.liver_get_cluster_data(adata, data_name, n_domains)
